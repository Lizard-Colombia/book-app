...
function MovieForm(props) {
  ...
  if (initialState.viewed === undefined) initialState.viewed = false;

  const [title, setTitle] = useState(initialState.title);
  const [rating, setRating] = useState(initialState.rating);
  const [releaseYear, setReleaseYear] = useState(initialState.releaseYear);
  const [viewed, setViewed] = useState(initialState.viewed);
  ...
  const onViewedChange = (event) => {
    setViewed(event.target.checked);
  };
  ...
  const onMovieSumbit = async (event) => {
    ...
    onSubmit(title, rating, releaseYear, viewed);
  };
...
{/* Viewed */}
<label className="movie-form__label">Movie Viewed? </label>
        <input
          className="movie-form__input"
          type="checkbox"
          value={viewed}
